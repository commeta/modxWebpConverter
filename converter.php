...
if ($json['mode'] == 'clean') { // Clean deleted copy of files into /webp/ directory
	if (!is_dir(BASE_PATH . DIRECTORY_SEPARATOR . 'webp'))
		goto die_clean;
...
if ($json['mode'] == 'convert') { // Converting *.jp[e]g and *.png files to /webp/[*/]*.webp
	if (isset($json['cwebp']) && $json['cwebp'] != 'gd') {
		if ($json['cwebp'] == 'system') {
			$cwebp = false;
			foreach (explode(':', getenv('PATH')) as $path_dir) {
				$candidate = rtrim($path_dir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'cwebp';

				if (is_executable($candidate)) {
					// Проверяем, что это действительно рабочий cwebp
					$test_output = [];
					$test_var = 0;
					exec($candidate . ' -version 2>&1', $test_output, $test_var);

					if ($test_var === 0) {
						$cwebp = $candidate;
						break;                    // ← важно! берём первый рабочий
					}
				}
			}

			if (!$cwebp) {
				_die(json_encode(['status' => 'cwebp not found in system PATH: ' . getenv('PATH')]));
			}
		} else {
	...
	if (is_array($bin)) { // Check binary
		foreach ($bin as $b) {
			if (is_file($cwebp_path . $b)) {
				if (!is_executable($cwebp_path . $b))
					chmod($cwebp_path . $b, 0755);
		...
		if (is_file($cwebp_path . $bin)) {
			if (strtolower(PHP_OS) != 'winnt' && !is_executable($cwebp_path . $bin))
				chmod($cwebp_path . $bin, 0755);
...
function recursive_search_img($dir)
{ // Search jpeg and png files recursive
	$odir = opendir($dir);
	...
	while (($file = readdir($odir)) !== FALSE) {
		...
		if (is_dir($full_path)) {
			foreach ($GLOBALS["excludeSubdirectories"] as $subdir) { // Exclude subdirectories from search
				if (strpos($full_path, BASE_PATH . $subdir) !== false)
					continue 2;
			}
			...
			if (
				...
				if (is_file($dest) && filemtime($full_path) < filemtime($dest))
					continue;
...
function recursive_search_webp($dir)
{ // Search webp files recursive
	$odir = opendir($dir);
...
function recursive_remove_empty_dirs($dir)
{ // Remove empty dirs
	$odir = opendir($dir);
	$count_files = 0;
...
{ // Prior to GD library version 2.2.5, WEBP does not have alpha channel support
	if (extension_loaded('gd') && function_exists('gd_info')) {
		...
		if (!in_array('WebP Support', $gd))
			$gd['WebP Support'] = 0;
		if (!in_array('JPEG Support', $gd))
			$gd['JPEG Support'] = 0;
		if (!in_array('PNG Support', $gd))
			$gd['PNG Support'] = 0;
...
class time_limit_exception
{ // Exit if time exceed time_limit
			...
			if ($json['mode'] == 'get') { //Too many files, use SSD instead HDD, or try again several times hoping for the system cache of the file system 
				...
				if (isset($images) && isset($cwebp)) {
					...
					if (json_last_error() != JSON_ERROR_NONE)
						_die(json_encode(['status' => 'Wrong filenames encoding!']));
					_die($ret);
				}
			}
		}
